...
def confidence_dynamic_ensemble(predictions_list, top_k=5):
    """신뢰도 기반 동적 앙상블"""
...
def main():
    ...
    for fold_idx in sorted(summary_df["fold"].unique()):
        ...
        model_dir = os.path.join(config.MODEL_OUTPUT_DIR, f"resnext_hierarchical_fold_{fold_idx}")
    ...
    conf_dynamic = confidence_dynamic_ensemble(fold_predictions, top_k=5)
