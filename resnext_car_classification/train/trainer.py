...
from resnext_car_classification.train.loss import FocalLoss, MixupLoss, AdaptiveClassWeightScheduler
from resnext_car_classification.train.sam import SAM, ESAM, LookSAM
from resnext_car_classification.dataset.augmentation import Mixup
from resnext_car_classification.utils.metrics import log_loss_calc_for_validation, multiclass_log_loss, \
...
class ImprovedHierarchicalTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if (step + 1) % self.config.GRADIENT_ACCUMULATION_STEPS == 0:
                if use_sam:
                    if self.scaler:
                        ...
                        # SAM first step - perturbation
                        self.optimizer.first_step(zero_grad=False)  # gradient 유지!

                        # Zero gradients for second forward
                        ...
                        with torch.amp.autocast(device_type='cuda', enabled=self.config.USE_FP16):
                            ...
                            if self.mixup and self.config.USE_MIXUP:
                                class_loss = self.mixup_class_criterion(outputs["logits"], labels_a, labels_b, lam) if \
                                outputs["class_loss"] is not None else torch.tensor(0.0)
            ...
            if class_loss is not None and isinstance(class_loss, torch.Tensor):
                epoch_class_loss += class_loss.item()
            if group_loss is not None and isinstance(group_loss, torch.Tensor):
                epoch_group_loss += group_loss.item()

            # 예측 수집 (Mixup이 없고 class loss가 활성화된 경우)
            ...
            if step % self.config.LOGGING_STEPS == 0:
                ...
                postfix = {
                    ...
                    "cls_l": f"{class_loss.item() if isinstance(class_loss, torch.Tensor) else 0:.4f}",
                    "grp_l": f"{group_loss.item() if isinstance(group_loss, torch.Tensor) else 0:.4f}"
                }
