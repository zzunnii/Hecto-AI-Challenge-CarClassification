...
class Config:
    ...
    MODEL_OUTPUT_DIR = r"./resnext_car_classification/outputs"  # 모델 아웃풋

    AUGMENTED_TRAIN_DIR = r"your path\augmented_data\fold_0\train_augmented"
    VAL_DIR = r"your path\open\augmented_data\fold_0\val"

    # 계층적 분류를 위한 매핑 파일
    MAPPING_CSV_PATH = r"your path\car_class_mapping.csv"

    OUTPUT_DIR = "./resnext_car_classification/outputs"

    # 모델 설정
    MODEL_NAME = "resnext50_32x4d"  # ResNet 계열 사용
    ...
    ATTENTION_DROPOUT = 0.3  # 어텐션 드롭아웃


    # 학습률 차등 적용
    USE_DIFFERENT_LR = True
    GROUP_LEARNING_RATE = 1e-4
    CLASS_LEARNING_RATE = 2e-4
    # 조기 정지 개선
    ...
    EARLY_STOPPING_PATIENCE = 10  # 조기 정지 인내심

    # SAM 관련
    USE_SAM = False  # SAM 사용 여부
    SAM_RHO = 0.2  # perturbation 크기
    ...
    # 학습 파라미터
    SEED = 42
    BATCH_SIZE = 64  # ResNet은 더 큰 이미지로 인해 배치 크기 줄임
    ...
    def get_model_type(self) -> str:
        """모델 타입 반환 - ResNeXt 우선 처리"""
        model_name_lower = self.MODEL_NAME.lower()

        # ResNeXt를 먼저 체크 (ResNet보다 구체적이므로)
        if 'resnext' in model_name_lower:
            return 'resnext'
