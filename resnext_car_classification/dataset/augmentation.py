...
def get_albumentations_transform(config, is_train=True):
    ...
    if is_train:
        ...
        transform_list.extend([
            ...
            A.OneOf([
                A.Compose([
                    A.Crop(x_min=0, y_min=0, x_max=width // 2, y_max=height, p=1.0),
                    A.LongestMaxSize(max_size=max(height, width), p=1.0),
                    A.PadIfNeeded(min_height=height, min_width=width,
                                  border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0)
                ], p=0.5),
                A.Compose([
                    A.Crop(x_min=width // 2, y_min=0, x_max=width, y_max=height, p=1.0),
                    A.LongestMaxSize(max_size=max(height, width), p=1.0),
                    A.PadIfNeeded(min_height=height, min_width=width,
                                  border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0)
                ], p=0.5),
            ...
            A.OneOf([
                A.Compose([
                    A.Crop(x_min=0, y_min=0, x_max=width, y_max=height // 2, p=1.0),
                    A.LongestMaxSize(max_size=max(height, width), p=1.0),
                    A.PadIfNeeded(min_height=height, min_width=width,
                                  border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0)
                ], p=0.5),
                A.Compose([
                    A.Crop(x_min=0, y_min=height // 2, x_max=width, y_max=height, p=1.0),
                    A.LongestMaxSize(max_size=max(height, width), p=1.0),
                    A.PadIfNeeded(min_height=height, min_width=width,
                                  border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0)
                ], p=0.5),
